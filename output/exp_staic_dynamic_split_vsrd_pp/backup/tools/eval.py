...
def eval_from_scrach_save(gt_dir, det_dir, ap_mode=40):
    ...
    if AP_mode == 40:
        saved_results['AP_Model'] = "AP40"
    if AP_mode == 11:
        saved_results['AP_Model'] = "AP11"
    
    saved_results['Cate'] = "Car"
...
def eval_from_scrach(gt_dir, det_dir, ap_mode=40):
    ...
    for i, f in enumerate(tqdm(all_f)):
        ...
        # assert gt['bbox'].all()==det['bbox'].all()

        '''alpha'''
        gt['alpha'] = gt_f[:, 3].astype(np.float32)
        det['alpha'] = det_f[:, 3].astype(np.float32)

        # assert gt['alpha'].all()==det['alpha'].all()
        ...
        # assert gt['occluded'].all()==det['occluded'].all()
        ...
        '''location'''
        gt['location'] = gt_f[:, 11:14].astype(np.float32)
        det['location'] = det_f[:, 11:14].astype(np.float32)+1e-5
        
        # assert gt['location'].all()==det['location'].all()

        '''dimensions, convert hwl to lhw'''
        gt['dimensions'] = gt_f[:, [10, 8, 9]].astype(np.float32)
        det['dimensions'] = det_f[:, [10, 8, 9]].astype(np.float32) +1e-5
        ...
        # assert gt['rotation_y'].all()==det['rotation_y'].all()
