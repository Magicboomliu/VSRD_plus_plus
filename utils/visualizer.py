...
def plot_3d(viz, pcd_1, clr_1, pcd_2, clr_2, dists, idxs, interactive=False):
    ...
    pcd_ren = o3d.geometry.PointCloud()
    ...
    # Build heatmap for distances
    heat_dists = rtools.build_heatmap(dists, min=0).astype(np.float32)

    # Build correspondence line set
    line_set = rtools.build_correspondence_lineset(
        pcd_2.detach().cpu().numpy(),
        pcd_1.detach().cpu().numpy(),
        idxs
    )
    if heat_dists.size != 0:
        line_set.colors = o3d.utility.Vector3dVector(np.array(heat_dists.squeeze()))
    ...
    if interactive:
        ...
        viz.add_geometry(pcd_patch)
        viz.add_geometry(pcd_ren)
        ...
        params.intrinsic.set_intrinsics(800, 600, 365.4020, 365.6674, 400 - 0.5, 300 - 0.5)
        pos = np.asarray(pcd_ren.points).mean(0)
        params.extrinsic = rtools.lookat(pos=pos - 1.5, target=pos)
        viz.get_view_control().convert_from_pinhole_camera_parameters(params)
        ...
        # Remove geometries after rendering
        viz.remove_geometry(pcd_patch)
        viz.remove_geometry(pcd_ren)
        viz.remove_geometry(line_set)
