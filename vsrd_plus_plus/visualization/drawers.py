...
def draw_boxes_3d(image, boxes_3d, line_indices, intrinsic_matrix, *args, **kwargs):
    ...
    for box_3d in boxes_3d:
        ...
        lines = box_3d[line_indices, ...] #(14,2,3): 12 +2, finaly 2 is for showing the dierection.
        
        # here the mask is instance level
        lines, masks = utils.numpy_function(operations.clip_lines_to_front)(lines) #(14,2,3)

        lines = lines @ intrinsic_matrix.T #(14,2,3)
        lines = lines[..., :-1] / np.clip(lines[..., -1:], 1e-3, None)#(14,2,2)
...
def draw_masks(image, masks, colors=None, weight=0.5):
    ...
    if colors is None:
        # colors is decided by the instance number randomly.
        colors = torch.rand(len(masks), 3).to(masks) #(N,3)
        colors = colors / torch.max(colors, dim=-1, keepdim=True).values

    colors = colors.T.reshape(3, -1, 1, 1) #(3,N,1,1) 
    
    masks = torch.sum(masks * colors, dim=1) #(3,N,H,W)--->(3,H,W)
