...
class Loss_Computation():
	...
	def prepare_targets(self, targets):
		...
		# confidence
		confidence3d = torch.stack([t.get_field("confidence3d") for t in targets])
	...
	def prepare_predictions(self, targets_variables, predictions):
		...
		batch_idxs = batch_idxs[flatten_reg_mask_gt.cpu()].to(reg_mask_gt.device) 
		...
		target_confidence_3d = targets_variables['confidence3d'].view(-1)[flatten_reg_mask_gt]
		...
		targets = { 'reg_2D': target_regression_2D, 'offset_3D': target_offset_3D, 'depth_3D': target_depths_3D, 'orien_3D': target_orientation_3D,
					...
					'confidence_3d': target_confidence_3d,
				}
	...
	def __call__(self, predictions, targets):
		...
		if num_reg_2D > 0:
			...
			nums_values = preds['reg_2D'].shape[0]
			conf_weights = pred_targets['confidence_3d'].unsqueeze(-1)
			conf_weights = conf_weights[:nums_values]
			
			
			reg_2D_loss, iou_2D = self.iou_loss(preds['reg_2D']*conf_weights, pred_targets['reg_2D']*conf_weights)
		...
		if num_reg_3D > 0:
			...
			offset_3D_loss = self.reg_loss_fnc(preds['offset_3D']*conf_weights, pred_targets['offset_3D']*conf_weights, reduction='none').sum(dim=1)
			...
			conf_weights = pred_targets['confidence_3d'].unsqueeze(-1)
			conf_weights = (conf_weights+1e-5)
			...
			# dimension loss
			nums_values = preds['dims_3D'].shape[0]
			conf_weights = pred_targets['confidence_3d'].unsqueeze(-1)
			conf_weights = conf_weights[:nums_values]

			dims_3D_loss = self.reg_loss_fnc(preds['dims_3D']*conf_weights, pred_targets['dims_3D']*conf_weights, reduction='none') * self.dim_weight.type_as(preds['dims_3D'])
			...
			if self.compute_corner_loss:
				# N x 8 x 3
				nums_values = preds['corners_3D'].shape[0]
				conf_weights = pred_targets['confidence_3d'].unsqueeze(-1).unsqueeze(-1)
				conf_weights = conf_weights[:nums_values]
