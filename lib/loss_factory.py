...
def calc_dis_ray_tracing(wl, Ry, points, density, bev_box_center):
    ...
    if dis_in_mul.size(0) == 0 or dis_all.size(0) == 0:
        return 0  # 如果 dis_in_mul 或 dis_all 是空张量，直接返回
...
def build_loss(pred_3D, batch_RoI_points, batch_lidar_y_center,
    ...
    for i in range(len(p_locXYZ)):
        ...
        dis_error = signed_loss(dis_error)
        center_loss = signed_loss(center_loss)
        ray_tracing_loss = signed_loss(ray_tracing_loss)
        center_yloss = signed_loss(center_yloss)
        orient_loss = signed_loss(orient_loss)
