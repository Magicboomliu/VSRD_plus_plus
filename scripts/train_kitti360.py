...
from config import cfg_kitti360_vsrdpp_splits as cfg
...
def train(cfg):
    ...
    exp_name = cfg.EXP_NAME # Resnet34
    layer = cfg.NET_LAYER # 34
    restore_epoch = cfg.RESTORE_EPOCH # Default is the None
    lr = cfg.TRAIN.LR  # 1e-4
    epochs = cfg.TRAIN.EPOCH #50 Epochs
    dim_prior = cfg.DATA.DIM_PRIOR ##[[1.6, 1.8, 4.0]]
    gt_dir = cfg.VAL.GT_DIR # validation GT Direction
    

    log.prepare_dirs(cfg) # Make the Log Folder and the checkpoint folder
    log.init_logger(cfg)
    ...
    TrainImgLoader_RoI = build_dataloader.build_train_kitti360_loader(cfg)
    InferImgLoader_RoI = build_dataloader.build_infer_kitti360_loader(cfg)
    ...
    for epoch_idx in range(start_epoch, epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader_RoI):
            ...
            if loss is None:
                ...
                if global_step%5 ==0:
                    logger.info("current loss is {}, iterations is {}".format(loss.data.item(),global_step))
...
def eval_one_epoch(save_dir_exp, InferImgLoader_RoI, model, dim_prior, gt_dir, ap_mode=40):
    ...
    with torch.no_grad():
        ...
        for batch_idx, sample in tqdm(enumerate(InferImgLoader_RoI)):
            ...
            det_3D[:, 8:11] = [np.array(dim_prior[0]) for _ in range(p_locXYZ.shape[0])]
