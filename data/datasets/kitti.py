...
class KITTIDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		if idx >= self.num_samples:
			...
			#objs = None if self.split == 'test' else self.get_label_objects(idx)
			objs = self.get_label_objects(idx)
		...
		# loaded the confidence here
		# confidence scores here
		confidence_3D = np.zeros([self.max_objs], dtype=np.float32) # N
				...
				if self.consider_outside_objs:
					...
					if self.proj_center_mode == 'intersect':
						try:
							...
							# print("Fuck!!!!!!!!!!!!!!!!!!!!!!!!")
							# print(target_proj_center.shape)
							# a = np.array([0,210.56894085])
							# print(a.shape)
						except:
							target_proj_center = np.array([0,210.56894085])
		...
		target.add_field("confidence3d",confidence_3D)
