...
# 2. Install all dependencies (creates an isolated env automatically)
pixi install

# 3a. Drop into an interactive shell with the env activated
pixi shell

# 3b. Or run a single command without activating
pixi run python main.py --help
```

> **CUDA version:** The default `pytorch-cuda = "11.8.*"` in `pixi.toml` requires a driver
> that supports CUDA 11.8.  Run `nvidia-smi` to check your driver version and edit the
> `pytorch-cuda` line in `pixi.toml` if you need a different version (e.g. `"12.1.*"` or
> `"12.4.*"`).

`xformers` is already included as an optional PyPI dependency in `pixi.toml` and will be
...
For custom stereo pair visualization, please check the [`demo`](demo) folder. You must comply with the NVIDIA license before using it.
