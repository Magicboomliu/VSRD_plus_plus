...
class BoxParameters3D_With_Scalar_Velocity(nn.Module):
    ...
    def __init__(
        ...
        self.register_parameter(
            "scalar_velocity",
            nn.Parameter(torch.ones(batch_size, num_instances, 1))
        )     
    ...
    def get_velocity_direction(self,orientations):
        current_orientations = nn.functional.normalize(orientations, dim=-1)
        cos_vector, sin_vector = torch.unbind(current_orientations,dim=-1)
        zero = torch.zeros_like(cos_vector)
        velocity_direction = torch.stack([sin_vector, zero,  cos_vector], dim=-1)        
    ...
    def forward(self):
        ...
        #velocity direction
        velocity_direction = self.get_velocity_direction(self.orientations)
