...
def import_module(config, globals=None, locals=None):
    # 检查 config 是否是包含函数的字典
    if isinstance(config, dict) and "function" in config:
        #导入指定的函数
        function = import_function(config.function)

        # 使用 map 函数递归地处理 config 中的 args 列表，将每个参数传递给 import_module 函数进行处理。
        # functools.partial 用于将 globals 和 locals 传递给 import_module 函数。
        args = list(map(functools.partial(import_module, globals=globals, locals=locals), config.get("args", [])))
        # 使用 map 函数递归地处理 config 中的 kwargs 字典，将每个值传递给 import_module 函数进行处理。
        # 使用 zip 函数将处理后的键值对组合成字典。
