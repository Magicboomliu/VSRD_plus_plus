...
if __name__=="__main__":
    ...
    resenet = ResnetEncoder(num_layers=18,pretrained=True)
    resenet = resenet.cuda()
    
    input_example = torch.randn(1,3,256,256).cuda()
    input_boundingbox_2d = torch.abs(torch.randn(1,3,4)).cuda()
    
    
    # [N,2]  [N,1]  [N,2]  
    location_xy, location_z, orientation_conf = resenet(input_example,input_boundingbox_2d)
