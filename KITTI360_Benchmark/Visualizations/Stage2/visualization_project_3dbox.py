...
def main(args):
    ...
    sync_file = os.path.join(root_folder,'testing/sync_file.txt')
    ...
    target_folder = sorted(os.listdir(pd_folder))
    ...
    for pd_label_name in tqdm(target_folder):
        
        pd_label_name_abs_path = os.path.join(pd_folder,pd_label_name)
        gt_label_name_abs_path = os.path.join(root_folder,'testing','label_gt',pd_label_name)
        ...
        for inner_idx, object in enumerate(est_obj_data):
            ...
            if args.options =="pd_gt":
                if inner_idx<len(gt_object_data):
                    ...
                    if gt_corners2d.min()<-10:
                        continue
                    copied_image_new = draw3d_bbox_2d_projection_V2(copied_image,qs=gt_corners2d,color=(0,255,0),is_gt=True)
                    if copied_image_new is not None:
                        copied_image_new = copied_image
                    else:
            ...
            if weakm3d_corners3d.min()<-0:
                continue
            copied_image_new = draw3d_bbox_2d_projection_V2(copied_image,qs=weakm3d_corners3d,color=(0,255,0),is_gt=False)
            if copied_image_new is not None:
                copied_image_new = copied_image
            else:
                copied_image = copied_image
