...
def draw_boxes_bev(image, boxes_3d, extents=((-40.0, 80.0), (40.0, 0.0)),
    ...
    for box_2d in boxes_2d:
        ...
        for (point_1, point_2) in zip(box_2d, np.roll(box_2d, shift=-1, axis=0)):
            ...
            if np.any(np.isnan(point_1)) or np.any(np.isnan(point_2)):
                print(f"Warning: Skipping invalid points: {point_1}, {point_2}")
                continue  # 跳过当前线段
...
def main(args):
    ...
    filenames = read_text_lines(args.filelist)

    stage2_vis_output_folder = args.stage2_vis_output_folder

    stage2_vis_output_folder_dynamic  = os.path.join(args.stage2_vis_output_folder,'dynamic')
    stage2_vis_output_folder_staic  = os.path.join(args.stage2_vis_output_folder,'static')
    
    os.makedirs(stage2_vis_output_folder,exist_ok=True)
    os.makedirs(stage2_vis_output_folder_dynamic,exist_ok=True)
    os.makedirs(stage2_vis_output_folder_staic,exist_ok=True)

    cnt = 0
    for pd_label_name in tqdm(sorted(os.listdir(pd_folder))):
        
        pd_label_name_abs_path = os.path.join(pd_folder,pd_label_name)
        gt_label_name_abs_path = os.path.join(root_folder,'training','label_gt',pd_label_name)
        left_image_abs_path = gt_label_name_abs_path.replace("label_gt","image_2").replace(".txt",'.png')
        calib_abs_path = gt_label_name_abs_path.replace("label_gt","calib")
        
        if pd_label_name[:-4] not in filenames:
            continue



        gt_data = sum(np.loadtxt(gt_label_name_abs_path,dtype=str).reshape(-1,17)[:,-1].astype(float))
        ...
        if dynamic:
            stage2_vis_output_folder = stage2_vis_output_folder_dynamic
        else:
            stage2_vis_output_folder = stage2_vis_output_folder_staic
            continue
        ...
        skimage.io.imsave(os.path.join(stage2_vis_output_folder,target_basename.replace(".txt",".png")),bevs)
...
if __name__=="__main__":
    ...
    parser.add_argument("--filelist",type=str, default="datasets/KITTI-360")
