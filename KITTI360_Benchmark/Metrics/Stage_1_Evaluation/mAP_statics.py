...
def eval_from_scrach_dynamic(gt_dir, det_dir, ap_mode=40,saved_fname=None,image_list=None):
    global AP_mode
    AP_mode = ap_mode
    all_gt, all_det = [], []
    all_f = sorted(os.listdir(det_dir))
    
    # set the various the all_f is OK
    all_f = sorted(os.listdir(det_dir))
    trainlist = read_text_lines(image_list)
    new_all_f = []
    for file in all_f:
        if file[:-4] in trainlist:
            new_all_f.append(file)
    all_f = new_all_f
    ...
    for i, f in enumerate(tqdm(all_f)):
        ...
        if len(read_text_lines(os.path.join(det_dir, f))[0].split())==16:
            det_f = np.loadtxt(os.path.join(det_dir, f), dtype=str).reshape(-1, 16)
        elif len(read_text_lines(os.path.join(det_dir, f))[0].split())==17:
            det_f = np.loadtxt(os.path.join(det_dir, f), dtype=str).reshape(-1, 17)
        else:
...
def eval_from_scrach_static(gt_dir, det_dir, ap_mode=40,saved_fname=None,image_list=None):
    global AP_mode
    AP_mode = ap_mode
    all_gt, all_det = [], []
    all_f = sorted(os.listdir(det_dir))
...
def eval_from_scrach(gt_dir, det_dir, ap_mode=40,saved_fname=None,image_list=None):
    global AP_mode
    AP_mode = ap_mode
    all_gt, all_det = [], []
