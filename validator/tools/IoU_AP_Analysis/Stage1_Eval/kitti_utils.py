...
def get_cars_from_label(label_file):
    with open(label_file, 'r') as f:
        lines = f.readlines()
    objects = [Object3d(line) for line in lines if line.strip().split(' ')[0]=='Car']
    return objects
...
class Object3d(object):
    def __init__(self, line):
        ...
        self.score = float(label[15]) if label.__len__()>15 else -1.0
        self.dynamic_label = float(label[16]) if label.__len__()>16 else -1.0
        self.level_str = None
    ...
    def generate_corners3d_raw(self,recti_matrix):
        """
        generate corners3d representation for this object
        :return corners_3d: (8, 3) corners of box3d in camera coord
        """
        ...
        corners3d = np.vstack([x_corners, y_corners, z_corners])  # (3, 8)
        corners3d = np.dot(R, corners3d).T
        corners3d = np.dot(corners3d,recti_matrix)
        corners3d = corners3d + self.pos
        return corners3d

    def to_bev_box2d_raw(self, oblique=True, voxel_size=0.1,rectifi_matrix = None):
        """
        ...
        if oblique:
            corners3d = self.generate_corners3d_raw(recti_matrix=rectifi_matrix)
            xz_corners = corners3d[0:4, [0, 2]]

            
            box2d = np.zeros((4, 2), dtype=np.int32)
            box2d[:, 0] = ((xz_corners[:, 0] - MIN_XZ[0]) / voxel_size).astype(np.int32)
            box2d[:, 1] = BEV_SHAPE[0] - 1 - ((xz_corners[:, 1] - MIN_XZ[1]) / voxel_size).astype(np.int32)
            
            box2d[:, 0] = np.clip(box2d[:, 0], 0, BEV_SHAPE[1])
            box2d[:, 1] = np.clip(box2d[:, 1], 0, BEV_SHAPE[0])
