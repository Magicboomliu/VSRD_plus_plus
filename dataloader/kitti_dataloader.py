...
class KITTI3D_Object_Dataset(torch.utils.data.Dataset):
    def __init__(self, cfg):
        self.KITTI_360_PATH = cfg.DATA.KITTI_360_PATH # root path
        self.cls_list = cfg.DATA.CLS_LIST # category path
...
def Processed_ROI_LiDAR(RoI_box_points):
    ...
    if len(boxes2d_list)==0:
        ...
        RoI_box_points['bbox2d'] = bboxes2d.cpu().numpy()
        RoI_box_points['bbox3d'] = bboxes3d.cpu().numpy()
...
class KITTI360_Object_Dataset_Raw_RoI_Lidar(KITTI3D_Object_Dataset):
    def __init__(self, cfg):
        super(KITTI360_Object_Dataset_Raw_RoI_Lidar, self).__init__(cfg)
        self.train_file = np.loadtxt(cfg.TRAIN.TRAIN_FILE, dtype=str)
        self.data_mode = cfg.DATA.MODE
        
        if self.data_mode =='KITTI360':
            self.img_list = np.array([os.path.join(cfg.DATA.KITTI_360_PATH, 'image_2', i+'.png') for i in self.train_file])
            self.calib_list = np.array([os.path.join(cfg.DATA.KITTI_360_PATH, 'calib', i+'.txt') for i in self.train_file])
    ...
    def __build_weights__(self, weight_path):
        if os.path.exists(weight_path):
            return
        weights = []
        for i in tqdm(self.lidar_RoI_points_path_list):
            weight = len(pickle.load(open(i, 'rb'))['bbox2d'])
    ...
    def __getitem__(self, index):
        ...
        if RoI_box_points is None:
            new_index = index +1
            if new_index < len(self.img_list)-1:
                return self.__getitem__(new_index)
            else:
                new_index = random.randint(0, len(self.img_list))
                return self.__getitem__(new_index)
        ...
        # conversion
        RoI_points = np.array(RoI_points,dtype=object)
        ...
        for i in range(bbox2d.shape[0]):
            '''dim'''    
            cls_dim_prior = self.dim_prior[0]
            batch_dim.append(cls_dim_prior)
            
            if RoI_points[i].ndim != 2 or RoI_points[i].shape[1] < 2:
                continue

            y_coor = RoI_points[i][:, 1]            
            if y_coor.size == 0:
                #print(f"Warning: Empty y_coor at index {i}")
                continue
            ...
            if y_coor.size > 0:  # if number is NAN
            ...
            # less than 2, 
            if y_ind_points.shape[0] < 2:
                #print(f"Warning: Insufficient points at index {i}")
                continue

            rand_ind = np.random.randint(0, y_ind_points.shape[0], 100) # defalut is 100
            ...
            '''orient'''
            orient_set = [
                (p1[1] - p2[1]) / (p1[0] - p2[0]) for p1 in depth_points_np_xz for p2 in depth_points_np_xz if p1[0] != p2[0]
            ]
            if len(orient_set) == 0:
                orient = 0
            else:
                orient_sort = np.array(sorted(orient_set))
                orient_sort = np.arctan(orient_sort[~np.isnan(orient_sort)])
                orient_sort_round = np.around(orient_sort, decimals=1)
                if len(orient_sort_round) > 0:
                    ind = np.argmax([np.sum(orient_sort_round == val) for val in set(orient_sort_round)])
                    orient = list(set(orient_sort_round))[ind]
                else:
                    orient = 0
...
def convert_roi_lidar_into_det_2d_format(bboxes2d):
    outside_loop = []
    for idx, line in enumerate(bboxes2d):
        inside_loop = []
        ...
        bottom = str(box2d[3])
        score = str(1.0)
        inside_loop.append(cls)
        inside_loop.append(left)
        inside_loop.append(top)
        inside_loop.append(right)
        inside_loop.append(bottom)
        inside_loop.append(score)
        outside_loop.append(inside_loop)
        
    return np.array(outside_loop)



class KITTI360_Object_Dataset_BBox2D(KITTI3D_Object_Dataset):
    def __init__(self, cfg):
        super(KITTI360_Object_Dataset_BBox2D, self).__init__(cfg)
        self.det_2D_path = cfg.VAL.SPLIT_FILE
        self.KITTI_360_PATH = cfg.DATA.KITTI_360_PATH
        
        self.INTERN_Image_2D_PATH = cfg.INFER.DET_2D_PATH
        
        self.infer_file = np.loadtxt(self.det_2D_path, dtype=str)
    ...
    def __getitem__(self, index):
        ...
        l_img_name = os.path.join(self.KITTI_360_PATH, 'image_2', file_name+'.png')
        l_img, bbox_shift = self.cut_or_pad_img(l_img_name, self.train_hw)

        # load calib files
        with open(os.path.join(self.KITTI_360_PATH, 'calib', file_name+'.txt'), encoding='utf-8') as f:
            text = f.readlines()
        ...
        internimage_2d_pred_path = os.path.join(self.INTERN_Image_2D_PATH, file_name+'.txt')
        internimage_instance_lidar_path = internimage_2d_pred_path.replace("det2d","ROI_LiDAR_InternImage").replace(".txt",".pkl")
        
        if not os.path.exists(internimage_2d_pred_path):
            with open(internimage_instance_lidar_path, 'rb') as f:
                RoI_box_points = pickle.load(f)
                RoI_boxes_2d = RoI_box_points['bbox2d']
                
                det_2D = convert_roi_lidar_into_det_2d_format(RoI_boxes_2d)
                bbox2d = RoI_boxes_2d.copy()
        else:
            
            det_2D = np.loadtxt(os.path.join(self.INTERN_Image_2D_PATH, file_name+'.txt'), dtype=str).reshape(-1, 6)
