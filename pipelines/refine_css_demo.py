...
def refine_css_demo(cfgp):
    ...
    precision = config.read_cfg_precision(cfgp, 'optimization', 'precision', default=torch.float16) # float16
    
    # Setup CSS
    css_path = config.read_cfg_string(cfgp, 'input', 'css_path', default='') # default settings: "data/nets/css.pt"
    ...
    label_type = config.read_cfg_string(cfgp, 'input', 'label_type', default='gt') # maskrcnn
    

    # Path for output autolabels
    path_autolabels = config.read_cfg_string(cfgp, 'output', 'labels', default='') # test_labels
