...
from vsrd_plus_plus import rendering

# projection
from vsrd_plus_plus.operations.geometric_operations import project_box_3d
from vsrd_plus_plus.operations.geometric_operations import rotation_matrix,rotation_matrix_x,rotation_matrix_y,rotation_matrix_z
from vsrd_plus_plus.operations.kitti360_operations import box_3d_iou
...
class MaskEroder(nn.Module):
    ...
    This can help improve model robustness to mask degradation.
    """
    def __init__(self, erode_ratio=0.05, threshold=0.5):
        """
        Args:
            erode_ratio: Erode 的百分比 (0.0-1.0)，例如 0.05 表示 5%
                        基于图像最小边长的百分比计算 kernel size
    ...
    def erode_single_mask(self, mask_np):
        """
        Erode a single mask using OpenCV.
        Args:
            mask_np: numpy array of shape (H, W), dtype uint8
        Returns:
            eroded_mask: numpy array of shape (H, W), dtype uint8
        """
        # 计算 kernel size（基于图像尺寸）
        H, W = mask_np.shape
        kernel_size = max(1, int(min(H, W) * self.erode_ratio))
        
        # 创建 kernel (必须是奇数，如果是偶数则+1)
        if kernel_size % 2 == 0:
            kernel_size += 1
        ...
        # 执行 erode
        eroded_mask = cv.erode(mask_np, kernel, iterations=1)
        
        return eroded_mask
    
    @utils.vectorize  # 自动处理 batch
    def erode_mask(self, mask):
        """
        Erode a mask tensor.
        Args:
            mask: torch.Tensor of shape (H, W) or (C, H, W)
        Returns:
            eroded_mask: torch.Tensor with same shape and dtype as input
        """
        # 保存原始设备
        original_device = mask.device
        original_dtype = mask.dtype
        
        # 转换为 numpy 并二值化 (得到 0 和 1 的 uint8)
        mask_np = (mask > self.threshold).cpu().numpy().astype(np.uint8)
        
        # 执行 erode (结果仍然是 0 和 1 的 uint8)
        eroded_mask_np = self.erode_single_mask(mask_np)
        
        # 确保是 numpy array（不是 Tensor）
    ...
    def forward(self, inputs):
        """
        Forward pass: erode masks in the inputs dictionary.
        """
        masks = inputs['masks']
...
def main(args=None):
    ...
    target_transforms = [
        Resizer(image_size=target_transforms_resize_size),
        MaskAreaFilter(min_mask_area=target_transforms_min_mask_area1),
        MaskRefiner(),
    ]
    
    # Add MaskEroder if erode_ratio > 0
    if args.erode_ratio > 0.0:
        target_transforms.append(MaskEroder(erode_ratio=args.erode_ratio))
        print(f"[MaskEroder] Enabled for target transforms with erode_ratio={args.erode_ratio}")
    
    target_transforms.extend([
    ...
    # Build source transforms
    source_transforms = [
        Resizer(image_size=source_transforms_resize_size),
    ...
    # Add MaskEroder if erode_ratio > 0
    if args.erode_ratio > 0.0:
        source_transforms.append(MaskEroder(erode_ratio=args.erode_ratio))
        print(f"[MaskEroder] Enabled for source transforms with erode_ratio={args.erode_ratio}")
    
    source_transforms.extend([
    ...
    def train():
        ...
        for multi_inputs in vsrd_plus_plus.distributed.tqdm(loaders):
            ...
            # Output Locations
            # Use custom checkpoint directory if provided, otherwise use default
            if args.ckpt_dirname:
                base_ckpt_dir = args.ckpt_dirname
            else:
                base_ckpt_dir = my_conf_train.TRAIN.CONFIG.replace("configs", "ckpts/{}".format(my_conf_train.TRAIN.MODEL_TYPE))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Custom checkpoint directory path. If not specified, uses default from config (configs -> ckpts/{MODEL_TYPE}).")
    
    parser.add_argument(
        "--log_dirname",
        type=str,
        default=None,
        help="Custom log directory path. If not specified, uses default from config (configs -> logs).")
    
    parser.add_argument(
        "--out_dirname",
        type=str,
        default=None,
        help="Custom output directory path. If not specified, uses default from config (configs -> outs).")
