...
def do_eval(cfg, model, data_loaders_val, iteration,code_path):
...
def do_train(
	...
	for data, iteration in zip(data_loader, range(start_iter, max_iter)):
		...
		if iteration % cfg.SOLVER.EVAL_INTERVAL == 0:
			...
			result_dict, result_str, dis_ious = do_eval(cfg, model, data_loaders_val, iteration,code_path=code_path)
			...
			if comm.get_rank() == 0:
				...
				important_key = '{}_3d_{:.2f}/moderate'.format('Car', 0.5)
